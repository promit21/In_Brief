...
import LatestNewsCardMbl from './LatestNewsCardMbl';
  ...
  return (
    <section className="container mx-auto">
      <header className="flex items-center justify-between mb-3">
        <h3 className="text-2xl font-medium ml-2">Latest News</h3>
        <span className="text-red-600 font-semibold mr-2">See All -&gt;</span>
      </header>

      <main className="grid grid-cols-1 md:grid-cols-4 gap-6 max-sm:hidden">
        {fakeData.map((item) => (
          <LatestNewsCard key={item.url} item={item} />
        ))}
      </main>

      <main className="sm:hidden">
        {fakeData.map((item) => (
          <LatestNewsCardMbl key={item.url} item={item} />
        ))}
      </main>
    </section>
  );
